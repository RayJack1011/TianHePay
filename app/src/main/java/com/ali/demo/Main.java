...
import com.ali.demo.api.domain.TradeFundBill;
import com.ali.demo.api.internal.utils.StringUtils;
import com.ali.demo.api.response.AlipayTradePrecreateResponse;
import com.ali.demo.api.response.AlipayTradeQueryResponse;
import com.ali.demo.trade.model.ExtendParams;
...
import com.ali.demo.trade.utils.Utils;
...
//            log.info("body:" + response.getBody());
    ...
    public void test_trade_pay(AlipayTradeService service) {
        ...
        String authCode = "289707718268970724"; // 条码示例，286648048691290423
...
//                .setExtendParams(extendParams)
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                System.out.println("info: 支付宝支付成功.)");
                break;
            case FAILED:
                System.out.println("error: 支付宝支付失败!!!");
                break;
            case UNKNOWN:
                System.out.println("error: 系统异常，订单状态未知!!!");
                break;
            default:
                System.out.println("error: 不支持的交易状态，交易返回异常!!!");
                break;
        }
    }
...
//                dumpResponse(response);
                if (Utils.isListNotEmpty(response.getFundBillList())) {
                    for (TradeFundBill bill : response.getFundBillList()) {
                        System.out.println("info" + bill.getFundChannel() + ":" + bill.getAmount());
                    }
                }
                break;
            case FAILED:
                System.out.println("error: 查询返回该订单支付失败或被关闭!!!");
                break;
            case UNKNOWN:
                System.out.println("error: 系统异常，订单支付状态未知!!!");
                break;
            default:
                System.out.println("error: 不支持的交易状态，交易返回异常!!!");
                break;
    ...
    public void test_trade_refund() {
        ...
        String outTradeNo = "tradepay15197193588628457992";
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                System.out.println("info: 支付宝退款成功: )");
                break;
            case FAILED:
                System.out.println("error: 支付宝退款失败!!!");
                break;
            case UNKNOWN:
                System.out.println("error: 系统异常，订单退款状态未知!!!");
                break;
            default:
                System.out.println("error: 不支持的交易状态，交易返回异常!!!");
                break;
    ...
    public void test_trade_precreate() {
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                System.out.println("info: 支付宝预下单成功: )");
                ...
                break;
            case FAILED:
                System.out.println("error: 支付宝预下单失败!!!");
                break;
            case UNKNOWN:
                System.out.println("error: 系统异常，预下单状态未知!!!");
                break;
            default:
                System.out.println("error: 不支持的交易状态，交易返回异常!!!");
                break;
